...
test_plottingDistance_line = testProperty "Line" $ \start end ->
    let line = Line start end
        (_gcode, PlottingState {_drawingDistance = drawnDistance}) = runPlotRaw def (plot line)
...
test_plottingDistance_circle = testProperty "Circle" $ \center (Positive radius) ->
    let circle = Circle center radius
        (_gcode, PlottingState {_drawingDistance = drawnDistance}) = runPlotRaw def (plot circle)
...
test_boundingBox_circle = testProperty "Circle" $ \center (Positive radius) ->
    let circle = Circle center radius
        (_gcode, PlottingState {_drawnBoundingBox = drawnBB}) = runPlotRaw def (plot circle)
        actual = drawnBB
...
test_boundingBox_arcCw90 = testCase "Arc 90° (clockwise)" $ do
    let start = zero
        ...
        (_gcode, PlottingState {_drawnBoundingBox = drawnBB}) = runPlotRaw def (repositionTo start >> clockwiseArcAroundTo center end)
...
test_boundingBox_arcCcw90 = testCase "Arc 90° (counter-clockwise)" $ do
    let start = zero
        ...
        actual = Actual drawnBB
        (_gcode, PlottingState {_drawnBoundingBox = drawnBB}) = runPlotRaw def (repositionTo start >> counterclockwiseArcAroundTo center end)
    assertApproxEqual "" expected actual
