...
data FirstStep
    = NoFirstStepPossible
    | FirstStepIs (Set Cube) Cube Cube
    deriving (Eq, Ord, Show)
...
pickStartAndFirstStep
    :: MWC.GenST s
    -> MoveConstraints
    -> (Set Cube, Circuits) -- ^ Possible starting points, existing circuits
    -> ST s FirstStep
...
fieldIsAllowed :: Cube -> Circuits -> MoveConstraints -> Bool
